...
class VerilogFileTemplate(object):
    ...
    wire_assign_template = 'assign {name} = {reference};'
...
class VerilogFileAST(_VerilogBaseAST):
    ...
    def generate(self, indent=0):
        ...
        assigns = '\n'.join(self._generate_map(self.assigns, indent + 1))
        if assigns:
            assigns += '\n\n'
        ...
        return VerilogFileTemplate.file_template.format(module_name=self.module_name,
                ports=ports, wires=wires, assigns=assigns,
                modules=modules, bd_name='{bd_name}')
...
class VerilogAssignAST(_VerilogBaseAST):
    def __init__(self):
        super().__init__()
        self.name = ''
        self.reference = ''
    ...
    def generate(self, indent=0):
        indent_str = self._indent * indent
        return indent_str + \
                VerilogFileTemplate.wire_assign_template.format(name=self.name,
